...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("assets/character.png",loader);
	ApplicationMain.total++;
	var loader1 = new flash.display.Loader();
	ApplicationMain.loaders.set("assets/platform.png",loader1);
	ApplicationMain.total++;
...
var Main = function() {
	...
	this.platformsOnStage = new List();
	this.textField = new flash.text.TextField();
	this.acc = new flash.sensors.Accelerometer();
	...
	this.characterContainer = new flash.display.Sprite();
	...
	this.get_stage().addEventListener(flash.events.KeyboardEvent.KEY_DOWN,$bind(this,this.onKeyDown));
	this.get_stage().addEventListener(flash.events.KeyboardEvent.KEY_UP,$bind(this,this.OnKeyUp));
	this.get_stage().addEventListener(flash.events.Event.ENTER_FRAME,$bind(this,this.OnEnterFrame));
	this.acc.addEventListener(flash.events.AccelerometerEvent.UPDATE,$bind(this,this.OnAcceleromterChange));
};
$hxClasses["Main"] = Main;
Main.__name__ = ["Main"];
...
var DefaultAssetLibrary = function() {
	...
	DefaultAssetLibrary.path.set("assets/character.png","assets/character.png");
	var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("assets/character.png",value);
	DefaultAssetLibrary.path.set("assets/platform.png","assets/platform.png");
	var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("assets/platform.png",value);
};
...
var MainCharacter = function() {
	flash.display.Sprite.call(this);
	var bitmap = new flash.display.Bitmap(openfl.Assets.getBitmapData("assets/character.png"));
	this.myGraphic = bitmap;
	this.addChild(bitmap);
	this.set_width(bitmap.get_width());
	this.set_height(bitmap.get_height());
};
$hxClasses["MainCharacter"] = MainCharacter;
MainCharacter.__name__ = ["MainCharacter"];
MainCharacter.__super__ = flash.display.Sprite;
MainCharacter.prototype = $extend(flash.display.Sprite.prototype,{
	__class__: MainCharacter
});
var IMap = function() { }
$hxClasses["IMap"] = IMap;
IMap.__name__ = ["IMap"];
...
var Platform = function() {
	flash.display.Sprite.call(this);
	var bitmap = new flash.display.Bitmap(openfl.Assets.getBitmapData("assets/platform.png"));
	this.myGraphic = bitmap;
	this.addChild(bitmap);
};
$hxClasses["Platform"] = Platform;
Platform.__name__ = ["Platform"];
Platform.__super__ = flash.display.Sprite;
Platform.prototype = $extend(flash.display.Sprite.prototype,{
	__class__: Platform
});
var PlatformManager = function() {
	this.platforms = new List();
	var _g = 0;
	while(_g < 20) {
		var i = _g++;
...
$hxClasses["PlatformManager"] = PlatformManager;
PlatformManager.__name__ = ["PlatformManager"];
PlatformManager.instance = function() {
	if(PlatformManager._instance == null) return PlatformManager._instance = new PlatformManager(); else return PlatformManager._instance;
}
...
flash.events.AccelerometerEvent = function(type,bubbles,cancelable,timestamp,accelerationX,accelerationY,accelerationZ) {
...
$hxClasses["flash.events.AccelerometerEvent"] = flash.events.AccelerometerEvent;
flash.events.AccelerometerEvent.__name__ = ["flash","events","AccelerometerEvent"];
flash.events.AccelerometerEvent.__super__ = flash.events.Event;
flash.events.AccelerometerEvent.prototype = $extend(flash.events.Event.prototype,{
	toString: function() {
	...
	,clone: function() {
		return new flash.events.AccelerometerEvent(this.type,this.bubbles,this.cancelable,this.timestamp,this.accelerationX,this.accelerationY,this.accelerationZ);
	}
	,__class__: flash.events.AccelerometerEvent
});
...
flash.sensors = {}
flash.sensors.Accelerometer = function() {
	flash.events.EventDispatcher.call(this);
	this._interval = 0;
	this.setRequestedUpdateInterval(flash.sensors.Accelerometer.defaultInterval);
};
$hxClasses["flash.sensors.Accelerometer"] = flash.sensors.Accelerometer;
flash.sensors.Accelerometer.__name__ = ["flash","sensors","Accelerometer"];
flash.sensors.Accelerometer.__properties__ = {get_isSupported:"get_isSupported"}
flash.sensors.Accelerometer.get_isSupported = function() {
	var supported = Reflect.hasField(js.Browser.window,"on" + "devicemotion");
	return supported;
}
flash.sensors.Accelerometer.__super__ = flash.events.EventDispatcher;
flash.sensors.Accelerometer.prototype = $extend(flash.events.EventDispatcher.prototype,{
	...
	,update: function() {
		var event = new flash.events.AccelerometerEvent(flash.events.AccelerometerEvent.UPDATE);
		var data = flash.display.Stage.__acceleration;
	...
	,setRequestedUpdateInterval: function(interval) {
		...
		if(this._interval < 0) throw new flash.errors.ArgumentError(); else if(this._interval == 0) this._interval = flash.sensors.Accelerometer.defaultInterval;
		if(this.timer != null) {
			this.timer.stop();
			this.timer = null;
		}
		if(flash.sensors.Accelerometer.get_isSupported() && !this.muted) {
	...
	,addEventListener: function(type,listener,useCapture,priority,useWeakReference) {
		...
		if(useCapture == null) useCapture = false;
		flash.events.EventDispatcher.prototype.addEventListener.call(this,type,listener,useCapture,priority,useWeakReference);
		this.update();
	}
	,__class__: flash.sensors.Accelerometer
...
flash.sensors.Accelerometer.defaultInterval = 34;
